...
ModelClass::ModelClass(string VertexShaderFileName, string FragmentShaderFileName, string objFileName,int Type)
{
	InitProgram(VertexShaderFileName, FragmentShaderFileName);
	if (Type == 0)
		LoadModel(objFileName);
	else if (Type == 1)
		LoadModelWithNormalMap(objFileName);
}
...
void ModelClass::LoadModel(string objFileName)
{
	#pragma region 宣告
...
void ModelClass::LoadModelWithNormalMap(string objFileName)
{
	#pragma region 宣告
	std::vector<tinyobj::shape_t> shapes;
	std::vector<tinyobj::material_t> materials;
	...
	model.matInfo.Shininess = .4 * 128;
	#pragma endregion
	#pragma region 要算出 Tangent & BiTangent
	vector<float> TangentArray;
	vector<float> BitangentArray;

	ComputeTangentBasis(shapes[0], TangentArray, BitangentArray);
	#pragma endregion
	#pragma region 綁點的資訊上 GPU
	//////////////////////////////////////////////////////////////////////////
	// 看一下其他章的，這種寫法 Shapes 只能有一個，超過一個畫無法畫出來 = =
	...
	// 上傳 Tangent
	glGenBuffers(1, &model.TangentBO);
	glBindBuffer(GL_ARRAY_BUFFER, model.TangentBO);
	glBufferData(GL_ARRAY_BUFFER, TangentArray.size() * sizeof(float), TangentArray.data(), GL_STATIC_DRAW);
	glEnableVertexAttribArray(3);
	glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, 0, NULL);

	// 上傳 Tangent
	glGenBuffers(1, &model.BitangentBO);
	glBindBuffer(GL_ARRAY_BUFFER, model.BitangentBO);
	glBufferData(GL_ARRAY_BUFFER, BitangentArray.size() * sizeof(float), BitangentArray.data(), GL_STATIC_DRAW);
...
void ModelClass::ComputeTangentBasis(tinyobj::shape_t shape, vector<float> &Tangent, vector<float> &Bitangent)
{
	int totalPoint = shape.mesh.positions.size();
	int totalCount = totalPoint / 9;
	for (int i = 0; i < totalCount; i++)
	{
		// Shortcuts for vertices
		vec3 v0 = vec3(shape.mesh.positions[9 * i + 0], shape.mesh.positions[9 * i + 1], shape.mesh.positions[9 * i + 2]);
		vec3 v1 = vec3(shape.mesh.positions[9 * i + 3], shape.mesh.positions[9 * i + 4], shape.mesh.positions[9 * i + 5]);
		vec3 v2 = vec3(shape.mesh.positions[9 * i + 6], shape.mesh.positions[9 * i + 7], shape.mesh.positions[9 * i + 8]);

		// Shortcuts for UVs
		vec2 uv0 = vec2(shape.mesh.texcoords[6 * i + 0], shape.mesh.texcoords[6 * i + 1]);
		vec2 uv1 = vec2(shape.mesh.texcoords[6 * i + 2], shape.mesh.texcoords[6 * i + 3]);
		vec2 uv2 = vec2(shape.mesh.texcoords[6 * i + 4], shape.mesh.texcoords[6 * i + 5]);
		...
		for (int j = 0; j < 3; j++)
		{
			Tangent.push_back(tangent[0]);
			Tangent.push_back(tangent[1]);
			Tangent.push_back(tangent[2]);

			Bitangent.push_back(bitangent[0]);
			Bitangent.push_back(bitangent[1]);
			Bitangent.push_back(bitangent[2]);
		}
	}
}

void ModelClass::BindMultiTexture(vector<string> ShaderName, vector<string> ImageName)
{
	glUseProgram(Program);
	for (int i = 0; i < ShaderName.size(); i++)
	{
		GLuint tempID = glGetUniformLocation(Program, ShaderName[i].c_str());
		glUniform1i(tempID, i);

		TextureData tdata = Load_png(("Images/" + ImageName[i]).c_str());

		GLuint TextureLoc;
		glGenTextures(1, &TextureLoc);
		glBindTexture(GL_TEXTURE_2D, TextureLoc);
		model.TextureID.push_back(TextureLoc);
...
void ModelClass::Draw(mat4 projM, mat4 viewM, mat4 modelM, vec3 lightPos)
{
	...
	for (size_t i = 0; i < model.TextureID.size(); i++)
	{
